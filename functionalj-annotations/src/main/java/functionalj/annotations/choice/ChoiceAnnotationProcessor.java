...
public class ChoiceAnnotationProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        for (Element element : roundEnv.getElementsAnnotatedWith(Choice.class)) {
            ...
            if (!isInterface) {
                error(element, "Only an interface can be annotated with " + Choice.class.getSimpleName() + ": " + simpleName);
                continue;
            }
            ...
            if ((specField != null) && !specField.matches("^[A-Za-z_$][A-Za-z_$0-9]*$")) {
                error(element, "Source spec field name is not a valid identifier: " + choiceType.specField());
                continue;
            }
    ...
    private List<Case> extractTypeChoices(Element element, Type targetType, TypeElement typeElement) {
        return typeElement.getEnclosedElements().stream()
                .filter(elmt->elmt.getKind().equals(ElementKind.METHOD))
                .map   (elmt->((ExecutableElement)elmt))
