...
/**
 * Annotation processor for SealedClass.
 * 
 * @author NawaMan -- nawa@nawaman.net
 */
public class SealedClassAnnotationProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        for (Element element : roundEnv.getElementsAnnotatedWith(Sealed.class)) {
            ...
            if (!isInterface) {
                error(element, "Only an interface can be annotated with " + Sealed.class.getSimpleName() + ": " + simpleName);
                continue;
            }
            ...
            val sealedClass    = element.getAnnotation(Sealed.class);
            val specTargetName = sealedClass.name();
            val targetName     = extractTargetName(simpleName, specTargetName);
            val targetType     = new Type(packageName, null, targetName, generics);
            
            val specField = emptyToNull(sealedClass.specField());
            if ((specField != null) && !specField.matches("^[A-Za-z_$][A-Za-z_$0-9]*$")) {
                error(element, "Source spec field name is not a valid identifier: " + sealedClass.specField());
                continue;
            }
            ...
            val publicFields = sealedClass.publicFields();
