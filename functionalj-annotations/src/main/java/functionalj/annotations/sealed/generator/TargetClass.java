...
import functionalj.annotations.sealed.CheckEquals;
import functionalj.annotations.sealed.SealedClassSwitch;
import functionalj.annotations.sealed.SealedClasses;
...
public class TargetClass implements Lines {
    ...
    public List<String> lines() {
        ...
        imports.add(SealedClassSwitch.class.getCanonicalName());
        imports.add("static " + SealedClasses.class.getCanonicalName() + ".Switch");
        imports.add("static " + CheckEquals.class.getCanonicalName()   + ".checkEquals");
        imports.add(AbstractSealedClass.class.getCanonicalName());
        ...
        if (spec.methods.stream().anyMatch(m -> Kind.DEFAULT.equals(m.kind))) {
            ...
            if (spec.sourceType.generics.isEmpty())
                 imports.add("functionalj.annotations.sealed.Self");
            else imports.add("functionalj.annotations.sealed.Self" + spec.sourceType.generics.size());
        }
        ...
        val specConstant = (spec.specObjName == null) ? "    " : "    public static final functionalj.annotations.sealed.generator.model.SourceSpec " + spec.specObjName + " = " + spec.toCode() + ";";
        return asList(
                ...
                asList(format("public abstract class %1$s extends AbstractSealedClass<%2$s.%2$sFirstSwitch%3$s> implements Pipeable<%4$s>%5$s {", typeWithGenericDef(), type.name, generics(), typeWithGenerics(), selfDef)),
                asList(format("    ")),
