...
public class IntegerAccessTest {
    ...
    public void testPrimitive() {
        ...
        val str1 = IntStreamPlus.infinite().map($I.time(2)).limit(1).sum();
        long actualUsedMem = memoryUsed(beforeUsedMem);
        System.out.println("actualUsedMem: " + actualUsedMem);
        val str2 = IntStreamPlus.infinite().mapToObj(i -> $I.time(2).apply(i)).limit(loop).mapToInt(i -> i).sum();
        ...
        val str3 = IntStreamPlus.infinite().map($I.time(2)).limit(loop).sum();
        ...
        val str4 = IntStreamPlus.infinite().mapToObj(i -> $I.time(2).apply(i)).limit(loop).mapToInt(i -> i).sum();
        ...
        val str5 = IntStreamPlus.infinite().map($I.time(2)).limit(loop).sum();
